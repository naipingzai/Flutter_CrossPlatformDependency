...
CC="${CC:-x86_64-w64-mingw32-gcc}"; AR="${AR:-ar}"; export CC AR
...
build_miniz() {
  ...
  ${cc} -shared -o "$inst/lib/libminiz.dll" "${objs[@]}"
...
build_stb_image() {
  ...
  ${cc} -shared -o "$inst/lib/libstb_image.dll" "$bd/stb_image.o"
...
build_sqlite() {
  ...
  ${cc} -shared -o "$inst/lib/libsqlite3.dll" "$bd/sqlite3.o"
