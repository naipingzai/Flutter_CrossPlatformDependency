...
CC="${CC:-$(xcrun --sdk macosx --find clang 2>/dev/null || echo clang)}"; AR="${AR:-ar}"; export CC AR; SYSROOT="${SYSROOT:-$(xcrun --sdk macosx --show-sdk-path 2>/dev/null || true)}"; export SYSROOT
...
build_python() {
  ...
  "${SRC_ROOT}/python/${DEP_SRC_DIR}/configure" --prefix="$inst" \
    --without-ensurepip --disable-shared --without-doc-strings --disable-test-modules 2>&1 || { echo "!!! [python] 编译失败，跳过" >&2; return 0; }
  make -j"$(platform_jobs)"; make install
  stage_lib python
